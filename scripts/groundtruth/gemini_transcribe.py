"""
Ground-truth transcription via Gemini.

Videos longer than CHUNK_SECONDS are split with ffmpeg before upload so
the model doesn't hallucinate past the ~5-minute mark.  Each chunk is
transcribed independently, timestamps are offset, then everything is merged.

Output goes to:  eval_results/groundtruth/transcription/gemini/<stem>.json
                 eval_results/groundtruth/transcription/gemini/<stem>.txt

Usage (single video):
    python -m groundtruth.gemini_transcribe \\
        --video data/videos/72694.mp4 --api-key KEY

Usage (batch):
    python -m groundtruth.gemini_transcribe \\
        --videos data/videos/72694.mp4 data/videos/74903.mp4 ... --api-key KEY
"""
...
OUTPUT_ROOT = Path("eval_results/groundtruth/transcription/gemini")
RPM_LIMIT = 3
...
def _split_audio(path: Path, chunk_secs: int, tmp_dir: str) -> list[tuple[Path, float]]:
    """Split video into audio-only mp3 chunks. Returns list of (chunk_path, start_offset_seconds)."""
...
def _extract_audio(path: Path, tmp_dir: str) -> Path:
    """Extract full audio track from a video as mp3."""
...
def _extract_text(response) -> str:
    """
    Gemini-3.5-transcribe returns audio_transcription parts, not plain text parts.
    Tries to use structured word-level timestamps first; falls back to transcript text.
    """
    # Happy path: normal text response
    try:
        t = response.text
        if t:
            return t
    ...
    for part in response.candidates[0].content.parts:
        ...
        # Try structured word-level timestamps → group into lines
        words = getattr(at, "words", None)
        if words:
            lines = _words_to_timestamped_lines(words)
            if lines:
                texts.append(lines)
                continue

        # Fall back to plain transcript field
        transcript = (
            getattr(at, "transcript", None)
            or getattr(at, "text", None)
            or str(at)
        )
        if transcript:
...
def _words_to_timestamped_lines(words, group_secs: float = 5.0) -> str:
    """
    Group word-level timestamp objects into ~5-second lines formatted as
    HH:MM:SS --> HH:MM:SS  text
    Works with both float seconds and Duration-proto style objects.
    """
    def _to_secs(v) -> float:
        if isinstance(v, (int, float)):
            return float(v)
        # Duration proto: has .seconds and .nanos
        s = getattr(v, "seconds", 0) or 0
        n = getattr(v, "nanos", 0) or 0
        return s + n / 1e9
    ...
    for w in words:
        ...
        start_secs = _to_secs(getattr(w, "start_time", 0) or getattr(w, "start_offset", 0) or 0)
        end_secs   = _to_secs(getattr(w, "end_time",   0) or getattr(w, "end_offset",   0) or 0)

        if group_start is None:
            group_start = start_secs

        group_words.append(word_text)
        group_end = end_secs

        # Flush when group reaches target duration
        if end_secs - group_start >= group_secs:
            lines.append(f"{_secs_to_ts(group_start)} --> {_secs_to_ts(group_end)}  {' '.join(group_words)}")
            group_words = []
            group_start = None
...
def _generate_with_retry(client, genai_types, file_uri: str, max_retries: int = 5):
    ...
    for attempt in range(max_retries):
        ...
        if elapsed < MIN_INTERVAL:
            ...
            print(f"    [rate limit] waiting {wait:.1f}s (3 RPM) ...", flush=True)
            time.sleep(wait)
        try:
...
def _transcribe_chunk(client, genai_types, chunk_path: Path, offset_secs: float) -> list[dict]:
    """Upload one audio chunk, get transcription, return segments with offset timestamps."""
    t0 = time.time()
    ...
    print(f"    [transcribe] done ({time.time()-t2:.1f}s)", flush=True)
    raw = _extract_text(response)
    print(f"    [extract] got {len(raw)} chars: {repr(raw[:120])}", flush=True)

    segments = _parse_segments(raw)
    # Shift timestamps by chunk start offset
    for seg in segments:
...
def transcribe_video(
    ...
    print(f"{video_path.name}  duration={duration:.1f}s  ({duration/60:.1f} min)")

    all_segments: list[dict] = []
    raw_parts: list[str] = []

    if duration <= CHUNK_SECONDS:
        print("  Single chunk — extracting audio ...")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            print(f"  Split into {len(chunks)} audio chunks of ~{CHUNK_SECONDS}s")
    ...
    print(f"  Saved {len(all_segments)} segments -> {json_out}")
    return result
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gemini ground-truth transcription")
    ...
    if args.all:
        ...
        pending = [v for v in all_videos if not (out_dir / f"{v.stem}.json").exists()]
