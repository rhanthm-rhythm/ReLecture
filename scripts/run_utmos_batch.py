"""
Batch UTMOS naturalness evaluation for the first N presentations (alphabetically).
Uses speechmos UTMOS22-full predictor. Loads the model once and reuses it.

Does NOT touch pipeline code. Only reads project manifests and segment WAV files.

Run this from the project root (Windows or WSL):
...
Results are written to:
  <output-root>/<videoID>/utmos.chatterbox.json
  <output-root>/_aggregate.json

Crash-safe: each result is written atomically to a .tmp file then renamed.
...
DEFAULT_OUTPUT_ROOT = ROOT_DIR / "eval_result" / "synthesis" / "UTMOS" / "chatterbox"
...
def _load_valid(path: Path) -> dict | None:
    ...
    try:
        ...
        _ = data["aggregates"]["utmos"]["mean"]  # must exist and be a number
        if data["aggregates"]["utmos"]["mean"] is None:
            return None
        return data
    except Exception:
        return None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch UTMOS naturalness evaluation using speechmos predictor."
    )
    parser.add_argument(
    ...
    # --- Load UTMOS predictor once ---
    try:
        from relecture.eval.utmos import _load_predictor
    except ImportError as exc:
        print(f"ERROR: could not import utmos module: {exc}", file=sys.stderr)
        sys.exit(1)

    print("Loading UTMOS predictor ...", flush=True)
    try:
        predictor = _load_predictor()
    except RuntimeError as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        sys.exit(1)
    print("UTMOS predictor ready.\n", flush=True)

    from relecture.eval.utmos import compute_utmos
    ...
    for idx, project_dir in enumerate(projects, 1):
        ...
        out_file = output_root / video_id / f"utmos.{args.tag}.json"
    ...
    for r in all_results:
        ...
        agg = r["aggregates"].get("utmos", {})
        row: dict = {
            "video_id": video_id,
            "utmos_mean": agg.get("mean"),
            "utmos_std": agg.get("std"),
    ...
    aggregate = {
        ...
        "corpus_utmos": _agg_stats(all_means),
    ...
    ca = aggregate["corpus_utmos"]
    if ca["mean"] is not None:
        print(f"Corpus UTMOS    : {ca['mean']:.4f}  (std={ca['std']:.4f}  n={ca['n']})")
    else:
        print("Corpus UTMOS    : N/A")
